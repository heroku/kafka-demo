  ...
  formatData(raw) {
    ...
    return _.transform(
      ...
      (res, values, topic) => {
        ...
        if (this._useInitial) {
          previous = _.map(
            _.last(this._initialData[topic]).relations,
            (r, name) => ({ name, r, topic })
          )
        }
        ...
        const topRelations = _.orderBy(
          relations,
          ['r', 'name'],
          ['desc', 'desc']
        )
        res.push(
          ...topRelations.slice(0, Math.floor(this.maxRelations / length))
        )
  ...
  init(data) {
    ...
    if (this._useInitial) {
      const startTime = new Date(
        Math.min(..._.map(data, (d) => _.last(d).time))
      )
  ...
  updateBubbles(options = {}) {
    ...
    const showTooltip = (d) => {
      ...
      this.tooltip
        ...
        .style(
          'left',
          `${d.x * scale + width / 2 - node.clientWidth / 2 + rect.left}px`
        )
        .style(
          'top',
          `${d.y * scale + height / 2 + d.r * scale + 2 + rect.top}px`
        )
    ...
    const updateCircles = updateSelection.select('circle')

    const updateText = updateSelection.select('text')
    ...
    enterSelection
      ...
      .attr(
        'transform',
        (d) =>
          `translate(${d.x * scale + center.x},${d.y * scale +
            center.y}) scale(${scale})`
      )
    ...
    enterText.merge(updateText).style('font-size', (d) => {
      const size =
        ((d.r * 2) / (this._textWidths[d.name] || 50)) * CALC_FONT_SIZE * 0.7
