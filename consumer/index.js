...
module.exports = class Consumer {
  constructor(options) {
    ...
    this._kafkaTopics = combine(
      this._topics,
      this._types,
      (topic, type) => `${topic.name}-${type.name}`
    )
    ...
    return Promise.all(
      this._consumers.map((consumer) =>
        consumer.init().then(() => {
          ...
          const { maxSize } = _.find(
            this._types,
            (type) => type.name === topic.split('-')[1]
          )
          if (maxSize && maxSize > 1) {
            ...
            return consumer
              .offset(topic)
              .then((latest) =>
                this._subscribe(consumer, { offset: latest - maxSize })
              )
          } else {
  ...
  snapshot() {
    return {
      type: 'snapshot',
      data: _.mapValues(this._snapshot, (type) =>
        _.mapValues(type, (data) => data.items())
      )
    }
  }
